...
export const changeMentor = async (req, res, next) => {
  try {
    ...
    if (student.mentor_name !== "Not yet assigned") {
      ...
      const newStudentList = previousMentor.students.filter(
...
export const assignMentor = async (req, res, next) => {
  try {
    ...
    if (student.mentor_name !== "Not yet assigned") {
      throw Error(
        `This student already has ${student.mentor_name} as his mentor, To change mentor for a student kindly use the appropriate end point`
      );
    }
...
export const getPreviousMentor = async (req, res, next) => {
  try {
    ...
    if (!student) {
      throw Error(
        "Student ID is not available in the database, Enter a valid ID"
      );
    }
