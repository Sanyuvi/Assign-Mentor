...
export const assignStudentToMentor = async (req, res, next) => {
  try {
    ...
    if (student.mentor_name !== "Not yet assigned") {
      throw Error(
        `This student already has ${student.mentor_name} as his mentor`
      );
    }
    ...
    res.status(201).json({
      success: true,
      mesage: `${mentor.name} has been assigned as a mentor to student ${student.name}`,
    });
  } catch (error) {
    next(error);
  }
      ...
      for (let id of students_id) {
        if ((await doc.student_id) === id) {
          if ((await doc.mentor_name) === "Not yet assigned") {
            studentArray = [
              ...studentArray,
              { name: doc.name, student_id: doc.student_id },
            ];
            break;
          }
...
export const getStudentsForMentor = async (req, res, next) => {
  try {
    ...
    if (!mentor) {
      throw Error(
        "Mentor name is not available in the database, Please enter a valid name"
      );
    }
